...
export function ArticleView({ v, c, fits, a, title, faq, related, path, verifiedAt }: {
  v: Vehicle; c: Category; fits: Fit[]; a: Article; title: string; faq: { q: string; a: string }[];
  related: GuideLink[]; path: string; verifiedAt?: string | null;
}) {
  ...
  return (
    <article className="art">
      ...
      <section className="box takeaways">
        ...
        <ol>{a.takeaways.map(t => <li key={t}><Inline s={t} /></li>)}</ol>
      </section>
      ...
      <section id="what-to-look-for">
        ...
        {a.look_for.map(x => <div key={x.h}><h3>{x.h}</h3><Md s={x.body} /></div>)}
      ...
      {a.install?.length ? (<section id="install"><h2>How to install a {c.name.toLowerCase().replace(/s$/, "")} on the {v.model_name}</h2><ol>{a.install.map(s => <li key={s}><Inline s={s} /></li>)}</ol></section>) : null}

      {a.avoid?.length ? (<section id="avoid"><h2>What to avoid</h2><div className="box warnbox">{a.avoid.map(x => <p key={x.h}><strong>{x.h}.</strong> {x.body}</p>)}</div></section>) : null}
      ...
      {faq.length > 0 && (<section id="faq"><h2>Frequently asked questions</h2>
        {faq.map(x => <details key={x.q} className="faq"><summary>{x.q}</summary><p>{x.a}</p></details>)}</section>)}
